...
def change_agent_decision(agent_id: int, payload: DecisionRequest, db=Depends(get_db)):
    ...
    if not agent:
        add_security_incident_log(
            ...
            description=f"Agent {agent_id} not found during decision making",
        )
        raise HTTPException(status_code=404, detail="Agent not Found")

    if (
        agent["risk_level"] == "high"
        and payload.decision == "allowed"
        and not payload.reason.strip()
    ):
        add_security_incident_log(
            cursor,
            incident_type="high_risk_agent",
            description=f"High risk agent {agent_id} was allowed without a reason",
        )
        raise HTTPException(
            status_code=400, detail="High Risk Agents require a reason to be allowed"
        )
