...
from agent_governance_console.backend.central_audit.insert_logs_into_schema import (
    add_security_incident_log,
    add_usage_ledger,
)
...
def log_agent_to_agent_usage(payload: UsageRequest, db=Depends(get_db)):
    ...
    if not target_row:
        add_security_incident_log(
            cursor,
            incident_type="missing_agent",
            description=f"Agent {payload.target} not found during usage logging for request {payload.request_id}",
    ...
    if not caller_row:
        add_security_incident_log(
            cursor,
            incident_type="missing_agent",
            description=f"Agent {payload.caller} not found during usage logging for request {payload.request_id}",
            caller_agent=payload.caller,
        )
        db.commit()
    ...
    if target_agent["status"] == "blocked":
        add_security_incident_log(
            cursor,
            incident_type="blocked_agent_call",
            description=f"Agent {payload.target} is blocked",
            target_agent=payload.target,
        )
        db.commit()
        raise HTTPException(
            status_code=403,
            detail=f"Agent {payload.target} is blocked from making calls",
        )
    ...
    if caller_agent["status"] == "blocked":
        add_security_incident_log(
            cursor,
            incident_type="blocked_agent_call",
            description=f"Agent {payload.caller} is blocked",
            caller_agent=payload.caller,
        )
        db.commit()
        raise HTTPException(
            status_code=403,
            detail=f"Agent {payload.caller} is blocked from making calls",
        )
    ...
    add_usage_ledger(
        ...
        caller_agent=payload.caller,
        target_agent=payload.target,
        caller_tokens_consumed=payload.caller_tokens,
        target_tokens_consumed=payload.target_tokens,
    )
    db.commit()
    cursor.execute(
        """
