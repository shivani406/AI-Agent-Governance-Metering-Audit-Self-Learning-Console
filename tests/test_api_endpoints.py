...
def test_get_all_agents(client):
    ...
    assert response.status_code == 200
    data = response.json()
    agents_list = data.get("agents") # api returns a dictionary always {"agents" : agents(list(dict)) }
...
@pytest.mark.parametrize("invalid_case, payload_patch", [
...
def test_apply_governance_decision_individual_validation_failures(client, invalid_case, payload_patch):
    base_payload = {
        "agent_id": 1,
        "decision": "blocked",
